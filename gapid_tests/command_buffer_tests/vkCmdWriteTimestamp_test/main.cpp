...
int main_entry(const entry::entry_data* data) {
  ...
  if (time_stamp_valid_bits == 0) {
...

    {
      ...
      cmd_buf->vkCmdResetQueryPool(cmd_buf, query_pool, 0, 2);
      ...
      cmd_buf->vkCmdWriteTimestamp(cmd_buf, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT,
                                   query_pool, 0);
    ...
    {
      // 2. Query time stamp for vkCmdDrawIndexed
      auto cmd_buf = app.GetCommandBuffer();
      app.BeginCommandBuffer(&cmd_buf);
      vulkan::RecordImageLayoutTransition(
          app.swapchain_images().front(),
      ...
      app.FillHostVisibleBuffer(
          ...
          &cmd_buf, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
      ...
      cmd_buf->vkCmdBindVertexBuffers(cmd_buf, 0, 2, vertex_buffers,
                                      vertex_buffer_offsets);
      cmd_buf->vkCmdBindIndexBuffer(cmd_buf, *index_buf, 0,
