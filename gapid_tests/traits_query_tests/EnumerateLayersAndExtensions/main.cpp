...
int main_entry(const entry::EntryData* data) {
  ...
  containers::vector<VkLayerProperties> layer_properties(data->allocator());
  layer_properties.resize(num_layers);

  if (num_layers > 1) {
    uint32_t num_reduced_layers = num_layers - 1;
    LOG_EXPECT(==, data->logger(), VK_INCOMPLETE,
  ...
  for (const auto& layer : layer_properties) {
    data->logger()->LogInfo("Instance Layer Found ", layer.layerName);
  ...
  for (const auto& physical_device : devices) {
    data->logger()->LogInfo("PhysicalDevice ", device_count++);
    LOG_EXPECT(==, data->logger(), VK_SUCCESS,
    ...
    for (const auto& layer : layer_properties) {
      data->logger()->LogInfo("  Device Layer Found ", layer.layerName);
