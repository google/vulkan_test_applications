...
#include "texel_buffer_alignment.vert.spv"
    ;

uint32_t cube_fragment_shader[] =
#include "texel_buffer_alignment.frag.spv"
    ;
...
struct CubeFrameData {
  ...
  containers::unique_ptr<vulkan::VkBufferView> uniform_buffer_view_;
  containers::unique_ptr<vulkan::VkBufferView> storage_buffer_view_;
};

VkPhysicalDeviceTexelBufferAlignmentFeaturesEXT kFeat{
...
class TexelBufferAlignmentSample
    : public sample_application::Sample<CubeFrameData> {
 public:
  TexelBufferAlignmentSample(const entry::EntryData* data)
      : data_(data),
  ...
  virtual void InitializeApplicationData(
    ...
    // Query texel buffer alignment properties, this is used below to
    // set the minimal offset alignments possible.
    VkPhysicalDeviceTexelBufferAlignmentPropertiesEXT texelProps{
        VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_TEXEL_BUFFER_ALIGNMENT_PROPERTIES_EXT,
        nullptr};
    VkPhysicalDeviceProperties2KHR deviceProperties2{
        VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2_KHR, &texelProps};
    ...
    if (!texelProps.storageTexelBufferOffsetSingleTexelAlignment) {
  ...
  virtual void InitializeFrameData(
...

    {
      VkBufferViewCreateInfo buffer_view_create_info{
          ...
          nullptr,                                           // pNext
          0,                                                 // flags
          uniform_data_->get_buffer(),                       // buffer
          VK_FORMAT_R8_UNORM,                                // format
          uniform_data_->get_offset_for_frame(frame_index),  // offset
          VK_WHOLE_SIZE,                                     // range
      };
      ::VkBufferView raw_buf_view;
      LOG_ASSERT(==, data_->logger(), VK_SUCCESS,
                 app()->device()->vkCreateBufferView(app()->device(),
                                                     &buffer_view_create_info,
                                                     nullptr, &raw_buf_view));
      frame_data->uniform_buffer_view_ =
...

    {
      VkBufferViewCreateInfo buffer_view_create_info{
          ...
          nullptr,                                           // pNext
          0,                                                 // flags
          storage_data_->get_buffer(),                       // buffer
          VK_FORMAT_R8_UNORM,                                // format
          storage_data_->get_offset_for_frame(frame_index),  // offset
          VK_WHOLE_SIZE,                                     // range
      };
      ::VkBufferView raw_buf_view;
      LOG_ASSERT(==, data_->logger(), VK_SUCCESS,
                 app()->device()->vkCreateBufferView(app()->device(),
                                                     &buffer_view_create_info,
                                                     nullptr, &raw_buf_view));
      frame_data->storage_buffer_view_ =
          containers::make_unique<vulkan::VkBufferView>(
              data_->allocator(),
              vulkan::VkBufferView(raw_buf_view, nullptr, &app()->device()));
    }
  ...
  virtual void Update(float time_since_last_render) override {
    ...
    for (int i = 0; i < 3; i++) {
      uniform_data_->data().data[i] =
          ((int)uniform_data_->data().data[i] + 1) % 256;
      storage_data_->data().data[i] =
          ((int)storage_data_->data().data[i] + 1) % 256;
    }
  }
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
    ...
    uniform_data_->UpdateBuffer(queue, frame_index);
    storage_data_->UpdateBuffer(queue, frame_index);
...
int main_entry(const entry::EntryData* data) {
  ...
  TexelBufferAlignmentSample sample(data);
  sample.Initialize();
