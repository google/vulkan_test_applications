...
class CubeSample : public sample_application::Sample<CubeFrameData> {
 public:
  ...
  virtual void InitializeApplicationData(
    ...
    NameObject(VK_OBJECT_TYPE_RENDER_PASS,
               uint64_t(render_pass_->get_raw_object()), "Main RenderPass");

    TagObject(VK_OBJECT_TYPE_RENDER_PASS,
              uint64_t(render_pass_->get_raw_object()), 0xbadf00dUL, 0,
              nullptr);
    ...
    NameObject(VK_OBJECT_TYPE_PIPELINE, uint64_t((VkPipeline)*cube_pipeline_),
               "Cube Pipeline");

    TagObject(VK_OBJECT_TYPE_PIPELINE, uint64_t((VkPipeline)*cube_pipeline_),
              0xdeadbeefUL, 0, nullptr);
    ...
    // Create, send a message and destroy a messenger.
    ...
    VkDebugUtilsMessengerCreateInfoEXT msg_create_info{
        ...
        DebugUtilsMessengerCallback,                         // pCallbackData
        nullptr                                              // pUserData
    };

    app()->instance()->vkCreateDebugUtilsMessengerEXT(
        app()->instance(), &msg_create_info, nullptr, &messenger);

    VkDebugUtilsMessengerCallbackDataEXT msg{
        VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CALLBACK_DATA_EXT,  // sType
        nullptr,                                                    // pNext
        0,                                                          // flags
        nullptr,        // pMessageIdName
        0,              // messageIdNumber
        "Hello World",  // pMessage
        0,              // queueLabelCount
        nullptr,        // pQueueLabels
        0,              // cmdBufLabelCount
        nullptr,        // pCmdBufLabels
        0,              // objectCount
        nullptr,        // pObjects
    };

    app()->instance()->vkSubmitDebugUtilsMessageEXT(
        app()->instance(), VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT,
        VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT, &msg);

    app()->instance()->vkDestroyDebugUtilsMessengerEXT(app()->instance(),
                                                       messenger, nullptr);
  }
  ...
  virtual void InitializeFrameData(
    ...
    VkDebugUtilsLabelEXT label{
        ...
        nullptr,                                  // pNext
        "Main Pass",                              // pLabelName
        {1.0f, 1.0f, 0.5f, 1.0f},                 // color
    };

    cmdBuffer->vkCmdBeginDebugUtilsLabelEXT(cmdBuffer, &label);
    ...
    label.pLabelName = "Cube";
    cmdBuffer->vkCmdInsertDebugUtilsLabelEXT(cmdBuffer, &label);
    ...
    cmdBuffer->vkCmdEndDebugUtilsLabelEXT(cmdBuffer);
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
    ...
    VkDebugUtilsLabelEXT label{
        VK_STRUCTURE_TYPE_DEBUG_UTILS_LABEL_EXT,  // sType
        nullptr,                                  // pNext
        "RenderQueueBegin",                       // pLabelName
        {0.0f, 1.0f, 0.5f, 1.0f},                 // color
    };

    app()->render_queue()->vkQueueBeginDebugUtilsLabelEXT(app()->render_queue(),
                                                          &label);

    label.pLabelName = "Important Q Operation";
    app()->render_queue()->vkQueueBeginDebugUtilsLabelEXT(app()->render_queue(),
                                                          &label);
    ...
    app()->render_queue()->vkQueueEndDebugUtilsLabelEXT(app()->render_queue());
  }
  ...
  void NameObject(VkObjectType type, uint64_t handle, const char* name) {
    ...
    app()->device()->vkSetDebugUtilsObjectNameEXT(app()->device(), &name_info);
  }

  void TagObject(VkObjectType type, uint64_t handle, uint64_t tag_name,
                 size_t tag_size, const void* tag) {
    VkDebugUtilsObjectTagInfoEXT tag_info{
        VK_STRUCTURE_TYPE_DEBUG_UTILS_OBJECT_TAG_INFO_EXT,  // sType
        nullptr,                                            // pNext
        type,                                               // objectType
        handle,                                             // objectHandle
        tag_name,                                           // tagName
    ...
    app()->device()->vkSetDebugUtilsObjectTagEXT(app()->device(), &tag_info);
  }
