...
class CubeSample : public sample_application::Sample<CubeFrameData> {
 public:
  ...
  virtual void InitializeApplicationData(
    ...
    size_t dataSize;
    if (app()->device()->vkGetShaderInfoAMD(
            app()->device(), *cube_pipeline_.get(),
            VK_SHADER_STAGE_FRAGMENT_BIT, VK_SHADER_INFO_TYPE_DISASSEMBLY_AMD,
            &dataSize, nullptr) == VK_SUCCESS) {
      app()->GetLogger()->LogInfo("Fragment shader disassembly: ");

      void* disassembly = malloc(dataSize);

      if (disassembly) {
        // Query disassembly and print
        app()->device()->vkGetShaderInfoAMD(
            app()->device(), *cube_pipeline_.get(),
            VK_SHADER_STAGE_FRAGMENT_BIT, VK_SHADER_INFO_TYPE_DISASSEMBLY_AMD,
            &dataSize, disassembly);
        app()->GetLogger()->LogInfo((char*)disassembly);
