...
VkPhysicalDeviceBufferDeviceAddressFeaturesKHR kBufferDeviceAddressFeatures = {
...
class CubeSample : public sample_application::Sample<CubeFrameData> {
 public:
  ...
  virtual void InitializeApplicationData(
    ...
    pipeline_layout_ = containers::make_unique<vulkan::PipelineLayout>(
        data_->allocator(),
        app()->CreatePipelineLayout({{cube_descriptor_set_layout_}}));
    ...
    buffer_address_data_ =
        containers::make_unique<vulkan::BufferFrameData<BufferAddressData>>(
            data_->allocator(), app(), num_swapchain_images,
            VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
    ...
    VkPhysicalDeviceBufferDeviceAddressFeaturesKHR ext_features = {
    ...
    app()->instance()->GetLogger()->LogInfo(
        "VK_KHR_buffer_device_address features:");
    app()->instance()->GetLogger()->LogInfo("bufferDeviceAddress: ",
                                            ext_features.bufferDeviceAddress);
    app()->instance()->GetLogger()->LogInfo(
        "bufferDeviceAddressCaptureReplay: ",
        ext_features.bufferDeviceAddressCaptureReplay);
    app()->instance()->GetLogger()->LogInfo(
        "bufferDeviceAddressMultiDevice: ",
        ext_features.bufferDeviceAddressMultiDevice);
  }
  ...
  virtual void InitializeFrameData(
    ...
    VkDescriptorBufferInfo buffer_info = {
        buffer_address_data_->get_buffer(),                       // buffer
        buffer_address_data_->get_offset_for_frame(frame_index),  // offset
        buffer_address_data_->size(),                             // range
    };
    ...
    VkBufferDeviceAddressInfoKHR address_info = {
        ...
        nullptr,
        camera_data_->get_buffer(),
    };
    VkDeviceAddress device_address =
        app()->device()->vkGetBufferDeviceAddressKHR(app()->device(),
                                                     &address_info);
    buffer_address_data_->data().camera_data_address =
        device_address + camera_data_->get_offset_for_frame(frame_index);

    address_info.buffer = model_data_->get_buffer();
    device_address = app()->device()->vkGetBufferDeviceAddressKHR(
        app()->device(), &address_info);
    buffer_address_data_->data().model_data_address =
        device_address + model_data_->get_offset_for_frame(frame_index);
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
    ...
    buffer_address_data_->UpdateBuffer(queue, frame_index);
 ...
 private:
  ...
  struct BufferAddressData {
    VkDeviceAddress camera_data_address;
    VkDeviceAddress model_data_address;
  };
  ...
  containers::unique_ptr<vulkan::BufferFrameData<BufferAddressData>>
      buffer_address_data_;
};
