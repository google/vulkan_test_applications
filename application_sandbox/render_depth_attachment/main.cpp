...
class RenderQuadSample
 ...
 public:
  ...
  virtual void InitializeApplicationData(
    ...
    // phase.
    auto o_att_desc = VkAttachmentDescription{
        0,                                        // flags
    ...
    o_att_desc.format = VK_FORMAT_D16_UNORM;
    o_att_desc.samples = VK_SAMPLE_COUNT_1_BIT;
    o_att_desc.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    o_att_desc.finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL;
    VkAttachmentReference depth_attachment = {
        0, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL};
    i_att_desc.format = VK_FORMAT_R8G8B8A8_UINT;
    i_att_desc.initialLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    i_att_desc.finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ...
    populateData(data_->logger(), depth_data_->data().data.data(),
                 depth_data_->data().data.size(), VK_FORMAT_R8G8B8A8_UINT,
                 VK_FORMAT_D16_UNORM);
  }
  ...
  virtual void InitializeFrameData(
    ...
    img_info.format = VK_FORMAT_D16_UNORM;
    ...
    // Image view for the depth attachment image
    ...
    // Create a framebuffer for rendering the final output
    ...
    (*frame_data->initial_rendering_command_buffer_)
        ->vkCmdCopyBufferToImage(
            *frame_data->initial_rendering_command_buffer_,
            depth_data_->get_buffer(), *frame_data->trans_dst_img_,
    ...
    // Renders the content in the color trans_dst images to the
    // depth attachment images.
