...
namespace icos_model {
#include "icos.obj.h"
}
const auto& icos_data = icos_model::model;

uint32_t icos_vertex_shader[] =
#include "blend.vert.spv"
    ;

uint32_t icos_fragment_shader[] =
#include "blend.frag.spv"
    ;

struct BlendFrameData {
 ...
 public:
  ...
  virtual void InitializeApplicationData(
    ...
    icos_pipeline_->AddShader(VK_SHADER_STAGE_VERTEX_BIT, "main",
                              icos_vertex_shader);
    icos_pipeline_->AddShader(VK_SHADER_STAGE_FRAGMENT_BIT, "main",
                              icos_fragment_shader);
    icos_pipeline_->AddDynamicState(VK_DYNAMIC_STATE_BLEND_CONSTANTS);
    icos_pipeline_->SetTopology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
    icos_pipeline_->SetInputStreams(&icos_);
    icos_pipeline_->SetViewport(viewport());
    icos_pipeline_->SetScissor(scissor());
    icos_pipeline_->SetSamples(num_samples());
    icos_pipeline_->AddAttachment(VkPipelineColorBlendAttachmentState{
        VK_TRUE, VK_BLEND_FACTOR_CONSTANT_COLOR, VK_BLEND_FACTOR_ZERO,
  ...
  virtual void InitializeFrameData(
    ...
    VkWriteDescriptorSet write{
        ...
        nullptr,                                 // pNext
        *frame_data->icos_descriptor_set_,       // dstSet
        0,                                       // dstbinding
        0,                                       // dstArrayElement
        2,                                       // descriptorCount
        VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,       // descriptorType
    ...
    cmdBuffer->vkCmdBindDescriptorSets(
        ...
        &frame_data->icos_descriptor_set_->raw_set(), 0, nullptr);
    icos_.Draw(&cmdBuffer);
