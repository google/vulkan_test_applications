...
layout (location = 1) out float distance_to_centerline;
...
void main() {
    gl_Position =  projection * transform * (get_position() + vec4(width * get_normal().xy, 0.0f, 0.0f));
    distance_to_centerline = get_normal().z;
}