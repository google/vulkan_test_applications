...
uint32_t diic_vertex_shader[] =
#include "diic.vert.spv"
    ;

uint32_t diic_fragment_shader[] =
#include "diic.frag.spv"
    ;

struct DrawIndexedIndirectCountData {
...
class DrawIndexedIndirectCountSample
 ...
 public:
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
                      DrawIndexedIndirectCountData* frame_data) override {
 ...
 private:
  ...
  containers::unique_ptr<vulkan::VulkanApplication::Buffer> drawDataBuffer;
  containers::unique_ptr<vulkan::VulkanApplication::Buffer> countDataBuffer;
};

int main_entry(const entry::EntryData* data) {
  data->logger()->LogInfo("Application Startup");
  DrawIndexedIndirectCountSample sample(data);
  sample.Initialize();
