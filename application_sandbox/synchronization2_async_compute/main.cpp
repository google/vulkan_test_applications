...
class ASyncThreadRunner {
 public:
  ...
  int32_t TryToReturnAndGetNextBuffer(int32_t index) {
    ...
    if (mailbox_buffers_.empty()) {
      // Nothing is ready;
      return index;
    }

    int32_t mb = mailbox_buffers_.back();
    mailbox_buffers_.pop_back();
    if (index != -1) {
 ...
 private:
  ...
  void PutBufferInMailbox(int32_t buffer) {
    std::lock_guard<std::mutex> lock(data_mutex_);
    mailbox_buffers_.push_back(buffer);
  }
  ...
  std::vector<int32_t> mailbox_buffers_;
  bool first = true;
  int current_frame = 0;
