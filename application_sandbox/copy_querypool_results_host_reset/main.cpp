...
class CopyQueryPoolHostResetResultSample
    : public sample_application::Sample<WireframeFrameData> {
 public:
  CopyQueryPoolHostResetResultSample(
  ...
  virtual void InitializeApplicationData(
    ...
    for (size_t i = 0; i < num_swapchain_images; i++) {
      app()->device()->vkResetQueryPoolEXT(app()->device(), *query_pool_,
  ...
  virtual void InitializeFrameData(
    ...
    // Copy the query results to be used in the next frame.
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
    ...
    // Previous execution of the commands using this frame's query must have
    // completed by now, so it's safe to reset.
    app()->device()->vkResetQueryPoolEXT(app()->device(), *query_pool_,
...
int main_entry(const entry::EntryData* data) {
  ...
  CopyQueryPoolHostResetResultSample sample(data, requested_features);
  sample.Initialize();
