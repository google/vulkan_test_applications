...
static uint64_t kPrivateDataValue = 12345L;

static VkDevicePrivateDataCreateInfoEXT private_data_reserve{
    VK_STRUCTURE_TYPE_DEVICE_PRIVATE_DATA_CREATE_INFO_EXT, nullptr, 1};

static VkPhysicalDevicePrivateDataFeaturesEXT ext_features{
    VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PRIVATE_DATA_FEATURES_EXT,
    &private_data_reserve, VK_TRUE};

class DataSlot {
 public:
  DataSlot(vulkan::VulkanApplication* application)
      : device_(application->device()) {
    VkPrivateDataSlotCreateInfoEXT slot_create_info = {
        VK_STRUCTURE_TYPE_PRIVATE_DATA_SLOT_CREATE_INFO_EXT, nullptr, 0};

    LOG_ASSERT(==, application->GetLogger(), VK_SUCCESS,
               device_->vkCreatePrivateDataSlotEXT(device_, &slot_create_info,
                                                   nullptr, &data_slot_));
  }

  ~DataSlot() {
    device_->vkDestroyPrivateDataSlotEXT(device_, data_slot_, nullptr);
  }

  VkPrivateDataSlotEXT get_handle() { return data_slot_; }

 private:
  VkPrivateDataSlotEXT data_slot_;
...
class CubeSample : public sample_application::Sample<CubeFrameData> {
 public:
      ...
      : data_(data),
        Sample<CubeFrameData>(
            ...
            sample_application::SampleOptions().AddDeviceExtensionStructure(
                &ext_features),
            {0}, {}, {VK_EXT_PRIVATE_DATA_EXTENSION_NAME}),
  ...
  virtual void InitializeApplicationData(
    ...
    data_slot_ = containers::make_unique<DataSlot>(data_->allocator(), app());

    LOG_ASSERT(==, app()->GetLogger(), VK_SUCCESS,
               app()->device()->vkSetPrivateDataEXT(
                   app()->device(), VK_OBJECT_TYPE_BUFFER,
                   (uint64_t)camera_data_->get_buffer(),
                   data_slot_->get_handle(), kPrivateDataValue));
  }
  ...
  virtual void Update(float time_since_last_render) override {
    uint64_t data = 0;
    app()->device()->vkGetPrivateDataEXT(app()->device(), VK_OBJECT_TYPE_BUFFER,
                                         (uint64_t)camera_data_->get_buffer(),
                                         data_slot_->get_handle(), &data);
    if (data != kPrivateDataValue) {
      return;
    }
 ...
 private:
  ...
  containers::unique_ptr<DataSlot> data_slot_;
