...
int main_entry(const entry::EntryData* data) {
    ...
    vulkan::VulkanApplication app(
        data->allocator(), data->logger(), data, {}, {}, {0}, 1024 * 1024,
        1024 * 1024, 1024 * 1024, 1024 * 1024, false, false, false, 0, false,
    ...
    while(!data->WindowClosing()) {
        ...
        post_ref_cmd->vkCmdBindDescriptorSets(
            ...
            0,
            1,
            &frameData[current_frame].descriptorSet->raw_set(),
            0,
            nullptr
        );
        ...
        LOG_ASSERT(==, data->logger(), VK_SUCCESS,
            app.EndAndSubmitCommandBuffer(
                ...
                &app.render_queue(),
                {
                    frameData[current_frame].imageAcquired->get_raw_object(), // wait for image
                    frameData[current_frame].gRenderFinished->get_raw_object(), // wait for gPass: not needed? maybe implicit synchro is enough.
                },
                {
                ...
                frameData[current_frame].renderingFence->get_raw_object() // frame fence
            )
        );

        VkSemaphore wait_semaphores[] = {frameData[current_frame].postRenderFinished->get_raw_object()};
