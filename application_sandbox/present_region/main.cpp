...
int main_entry(const entry::EntryData* data) {
  ...
  while (true) {
    ...
    VkRectLayerKHR rect{
        {static_cast<int32_t>(sin(w) * app.swapchain().width() / 5.0f +
                              app.swapchain().width() / 2.0f),
         static_cast<int32_t>(cos(w) * app.swapchain().height() / 5.0f +
                              app.swapchain().height() / 2.0f)},
        {20, 20},
        0};
    ...
    VkPresentRegionsKHR presentRegions{
        VK_STRUCTURE_TYPE_PRESENT_REGIONS_KHR,  // sType
        nullptr,                                // pNext
        1,                                      // swapchainCount
        &region                                 // pRegions
    };
