...
#include "bufferview.vert.spv"
    ;

uint32_t cube_fragment_shader[] =
#include "bufferview.frag.spv"
    ;
...
struct CubeFrameData {
  ...
  containers::unique_ptr<vulkan::VkBufferView> alpha_data_buffer_view_;
};

// This creates an application with 16MB of image memory, and defaults
// for host, and device buffer sizes.
class BufferViewSample : public sample_application::Sample<CubeFrameData> {
 public:
  BufferViewSample(const entry::EntryData* data)
      : data_(data),
  ...
  virtual void InitializeFrameData(
    ...
    VkBufferViewCreateInfo alpha_data_buffer_view_create_info{
        VK_STRUCTURE_TYPE_BUFFER_VIEW_CREATE_INFO,       // sType
        nullptr,                                         // pNext
        0,                                               // flags
        alpha_data_->get_buffer(),                       // buffer
        VK_FORMAT_R32_SFLOAT,                            // format
        alpha_data_->get_offset_for_frame(frame_index),  // offset
        VK_WHOLE_SIZE,                                   // range
    };
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
    ...
    alpha_data_->UpdateBuffer(queue, frame_index);
