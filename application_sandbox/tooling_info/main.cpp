...
class CubeSample : public sample_application::Sample<CubeFrameData> {
 public:
  ...
  virtual void InitializeApplicationData(
    ...
    PFN_vkGetPhysicalDeviceToolPropertiesEXT
        VkGetPhysicalDeviceToolPropertiesEXT =
            reinterpret_cast<PFN_vkGetPhysicalDeviceToolPropertiesEXT>(
                app()->instance()->getProcAddr(
                    app()->instance().get_instance(),
                    "vkGetPhysicalDeviceToolPropertiesEXT"));

    if (nullptr == VkGetPhysicalDeviceToolPropertiesEXT) {
      app()->GetLogger()->LogError(
          "vkGetPhysicalDeviceToolPropertiesEXT not found!");
      return;
    }

    uint32_t tool_count = 0;
    VkGetPhysicalDeviceToolPropertiesEXT(app()->device().physical_device(),
                                         &tool_count, nullptr);

    app()->GetLogger()->LogInfo("Tooling Info returned ", tool_count,
                                " active tools.");
    if (tool_count == 0) {
      return;
    }
    ...
    VkGetPhysicalDeviceToolPropertiesEXT(app()->device().physical_device(),
                                         &tool_count, &tool_properties[0]);

    const char* tool_purposes[] = {"VALIDATION",         "PROFILING",
                                   "TRACKING",           "ADDITIONAL_FEATURES",
                                   "MODIFYING_FEATURES", "DEBUG_REPORTING",
                                   "DEBUG_MARKERS"};

    for (uint32_t tool = 0; tool < tool_count; tool++) {
      app()->GetLogger()->LogInfo("Tool #", tool, ":");
      app()->GetLogger()->LogInfo("  Name: ", tool_properties[tool].name);
      app()->GetLogger()->LogInfo("  Version: ", tool_properties[tool].version);

      for (uint64_t i = 0; i < sizeof(tool_purposes) / sizeof(tool_purposes[0]);
           i++) {
        if (tool_properties[tool].purposes & (1 << i)) {
          app()->GetLogger()->LogInfo("  Purpose: ", tool_purposes[i]);
        }
      }

      app()->GetLogger()->LogInfo("  Description: ",
                                  tool_properties[tool].description);
      app()->GetLogger()->LogInfo("  Layer: ", tool_properties[tool].layer);
    }
  }
