...
class DescriptorUpdateTemplateSample
    : public sample_application::Sample<CubeFrameData> {
 public:
  DescriptorUpdateTemplateSample(const entry::EntryData* data)
      : data_(data),
  ...
  virtual void InitializeFrameData(
    ...
    // This entry is for the two VkDescriptorBufferInfo stored in the
    // buffer_infos array just declared above, hence the descriptorCount of 2
    // and the stride value of sizeof(VkDescriptorBufferInfo). descriptorCount
    // is generally use for index of array descriptors (as in, the descriptor
    // refers to an array in its declaration in the shader), but it can be also
    // used to refer to several successive bindings, as it is the case here. See
    // https://www.khronos.org/registry/vulkan/specs/1.1-extensions/man/html/VkWriteDescriptorSet.html
    // in the Description, the paragraph about dstBinding having less elements
    // than descriptorCount. The same applies here.
    VkDescriptorUpdateTemplateEntry descriptor_update_template_entry = {
        ...
        VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,  // descriptorType
        0,                                  // offset
        sizeof(VkDescriptorBufferInfo),     // stride
    };
    ...
    VkDescriptorUpdateTemplateCreateInfo descriptor_update_template_create_info = {
        ...
        &descriptor_update_template_entry,  // pDescriptorUpdateEntries
        VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_DESCRIPTOR_SET,  // templateType
        frame_data->cube_descriptor_set_->layout(),  // descriptorSetLayout
        // pipelineBindPoint, ignored here because templateType is not VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_PUSH_DESCRIPTORS_KHR
        (VkPipelineBindPoint)0,
        // pipelineLayout, ignored here because templateType is not VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_PUSH_DESCRIPTORS_KHR
        (VkPipelineLayout)0,
        // set, ignored here because templateType is not VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_PUSH_DESCRIPTORS_KHR
        0,
    };
    ...
    LOG_ASSERT(==, app()->GetLogger(),
               app()->device()->vkCreateDescriptorUpdateTemplate(
                   app()->device(), &descriptor_update_template_create_info,
                   nullptr, &descriptor_update_template),
               VK_SUCCESS);


    app()->device()->vkUpdateDescriptorSetWithTemplate(
        app()->device(), frame_data->cube_descriptor_set_->raw_set(),
        descriptor_update_template, (const void*)buffer_infos);
...
int main_entry(const entry::EntryData* data) {
  ...
  DescriptorUpdateTemplateSample sample(data);
  sample.Initialize();
