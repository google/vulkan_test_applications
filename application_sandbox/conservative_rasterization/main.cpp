...
struct ConservativeRasterizationFrameData {
...
class ConservativeRasterizationSample
    : public sample_application::Sample<ConservativeRasterizationFrameData> {
 public:
  ConservativeRasterizationSample(const entry::EntryData* data)
      : data_(data),
        Sample<ConservativeRasterizationFrameData>(
  ...
  virtual void InitializeApplicationData(
    ...
    conservative_rasterization_pipeline_->SetViewport(viewport());
    conservative_rasterization_pipeline_->SetScissor(scissor());
    ...
    conservativeRasterState.extraPrimitiveOverestimationSize =
        conservativeRasterProps.maxExtraPrimitiveOverestimationSize;

    conservative_rasterization_pipeline_->SetRasterizationExtension(
        &conservativeRasterState);

    conservative_rasterization_pipeline_->Commit();
    ...
    model_data_->data().transform =
        ...
        Mat44::FromScaleVector(
            mathfu::Vector<float, 3>{STICK_THICKNESS, 3.0f, STICK_THICKNESS});
  }

  virtual void InitializeFrameData(
      ConservativeRasterizationFrameData* frame_data,
      vulkan::VkCommandBuffer* initialization_buffer,
  ...
  virtual void Update(float time_since_last_render) override {
    model_data_->data().transform =
        model_data_->data().transform *
        Mat44::FromScaleVector(mathfu::Vector<float, 3>{
            1.0f / STICK_THICKNESS, 1.0f / 3.0f, 1.0f / STICK_THICKNESS}) *
        Mat44::FromRotationMatrix(
            Mat44::RotationX(3.14f * time_since_last_render) *
            Mat44::RotationY(3.14f * time_since_last_render * 0.5f) *
            Mat44::RotationZ(3.14f * time_since_last_render * 0.5f)) *
        Mat44::FromScaleVector(
            mathfu::Vector<float, 3>{STICK_THICKNESS, 3.0f, STICK_THICKNESS});
  }
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
                      ConservativeRasterizationFrameData* frame_data) override {
...
int main_entry(const entry::EntryData* data) {
  ...
  ConservativeRasterizationSample sample(data);
  sample.Initialize();
