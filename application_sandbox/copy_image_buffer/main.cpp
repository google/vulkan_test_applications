...
struct CubeFrameData {
  ...
  vulkan::BufferPointer stage_buf_;  // The staging buffer, render result is
                                     // copied to this buffer and then copied to
                                     // swapchain image.
};
...
class CopyImageBufferSample : public sample_application::Sample<CubeFrameData> {
 public:
  ...
  virtual void InitializeFrameData(
    ...
    VkBufferMemoryBarrier as_dst_buf{
        ...
        VK_QUEUE_FAMILY_IGNORED,                  // srcQueueFamilyIndex
        VK_QUEUE_FAMILY_IGNORED,                  // dstQueueFamilyIndex
        *frame_data->stage_buf_,                  // buffer
        0,                                        // offset
        VK_WHOLE_SIZE                             // size
    };
    VkBufferMemoryBarrier as_src_buf{
        VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,  // sType
        nullptr,                                  // pNext
    ...
    copy_region.imageOffset = {int(app()->swapchain().width() / 3),
                               int(app()->swapchain().height() / 3), 0};
    cmdBuffer->vkCmdCopyBufferToImage(
        cmdBuffer, *frame_data->stage_buf_, swapchain_image(frame_data),
