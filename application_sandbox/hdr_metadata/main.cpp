...
#include "hdr_metadata.vert.spv"
    ;

uint32_t torus_fragment_shader[] =
#include "hdr_metadata.frag.spv"
    ;

struct HdrMetadataFrameData {
...
class HdrMetadataSample
    : public sample_application::Sample<HdrMetadataFrameData> {
 public:
  HdrMetadataSample(const entry::EntryData* data,
                    VkPhysicalDeviceFeatures& requested_features)
      : data_(data),
        Sample<HdrMetadataFrameData>(
            data->allocator(), data, 1, 512, 1, 1,
            sample_application::SampleOptions()
                .EnableDepthBuffer()
                .Enable10BitHDR(),
            requested_features, {VK_EXT_SWAPCHAIN_COLOR_SPACE_EXTENSION_NAME},
  ...
  virtual void InitializeFrameData(
      HdrMetadataFrameData* frame_data,
      vulkan::VkCommandBuffer* initialization_buffer,
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
                      HdrMetadataFrameData* frame_data) override {
...
int main_entry(const entry::EntryData* data) {
  ...
  HdrMetadataSample sample(data, requested_features);
  sample.Initialize();
