...
static VkPhysicalDevice16BitStorageFeatures sixteen_bit_storage_feature{
...
class CubeSample : public sample_application::Sample<CubeFrameData> {
 public:
  ...
  virtual void InitializeApplicationData(
    ...
    VkPhysicalDevice16BitStorageFeatures ext_features = {
        VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_16BIT_STORAGE_FEATURES};
    ...
    app()->instance()->GetLogger()->LogInfo("VK_KHR_16bit_storage features:");
    app()->instance()->GetLogger()->LogInfo(
        "storageBuffer16BitAccess: ", ext_features.storageBuffer16BitAccess);
    app()->instance()->GetLogger()->LogInfo(
        "uniformAndStorageBuffer16BitAccess: ",
        ext_features.uniformAndStorageBuffer16BitAccess);
    app()->instance()->GetLogger()->LogInfo("storagePushConstant16: ",
                                            ext_features.storagePushConstant16);
    app()->instance()->GetLogger()->LogInfo("storageInputOutput16: ",
                                            ext_features.storageInputOutput16);
  ...
  virtual void Update(float time_since_last_render) override {
    ...
    uint16_t v = uint16_t(std::abs((int32_t(frame_count) % 512) - 255));
    color_data_->data().r = v;
    color_data_->data().g = v;
    color_data_->data().b = v;
  }
...
int main_entry(const entry::EntryData* data) {
  ...
  CubeSample sample(data, request_features);
  sample.Initialize();
