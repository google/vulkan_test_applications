...
layout(set = 0, binding = 4) uniform texture2D default_texture2;

void main() {
    vec4 color = texture(sampler2D(default_texture, default_sampler), texcoord);
    vec4 color2 = texture(sampler2D(default_texture2, default_sampler), texcoord);
    if(texcoord.x > texcoord.y) {
        out_color = vec4(color.xyz, 1.0);
    } else {
        out_color = vec4(color2.xyz, 1.0);
    }
}