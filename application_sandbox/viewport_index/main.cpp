...
class ViewportIndexSample : public sample_application::Sample<CubeFrameData> {
 public:
  ViewportIndexSample(const entry::EntryData* data,
  ...
  virtual void InitializeApplicationData(
    ...
    float width = static_cast<float>(app()->swapchain().width());
    float height = static_cast<float>(app()->swapchain().height());

    VkViewport viewports[4] = {
        {0.0f, 0.0f, width / 2.0f, height / 2.0f, 0.0f, 1.0f},
        {width / 2.0f, 0.0f, width / 2.0f, height / 2.0f, 0.0f, 1.0f},
        {0.0f, height / 2.0f, width / 2.0f, height / 2.0f, 0.0f, 1.0f},
        {width / 2.0f, height / 2.0f, width / 2.0f, height / 2.0f, 0.0f, 1.0f},
    };

    VkRect2D scissors[4] = {scissor(), scissor(), scissor(), scissor()};
    ...
    cube_pipeline_->SetViewports(viewports, 4);
    cube_pipeline_->SetScissors(scissors, 4);
...
int main_entry(const entry::EntryData* data) {
  ...
  request_features.multiViewport = VK_TRUE;
  ViewportIndexSample sample(data, request_features);
  sample.Initialize();
