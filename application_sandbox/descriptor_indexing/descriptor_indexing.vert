...
layout (location = 2) flat out int tex_i;

layout (binding = 0, set = 0) buffer camera_data {
...
void main() {
    gl_Position =  projection[gl_InstanceIndex] * transform * get_position();
    texcoord = get_texcoord();
    tex_i = gl_InstanceIndex;
}