...
struct CalibratedTimestampsFrameData {
  ...
  containers::vector<VkTimeDomainEXT*> time_domains_;
};

std::string TimeDomainToString(VkTimeDomainEXT d) {
  switch (d) {
...
class CalibratedTimestampsSample
    : public sample_application::Sample<CalibratedTimestampsFrameData> {
 public:
  CalibratedTimestampsSample(const entry::EntryData* data)
      : data_(data),
        Sample<CalibratedTimestampsFrameData>(
            data->allocator(), data, 1, 512, 1, 1,
            sample_application::SampleOptions(), {}, {},
            {VK_EXT_CALIBRATED_TIMESTAMPS_EXTENSION_NAME}),
        cube_(data->allocator(), data->logger(), cube_data),
        time_domains_(data_->allocator()),
        frame_count_(0) {}
  virtual void InitializeApplicationData(
    ...
    VkResult result =
        app()->instance()->vkGetPhysicalDeviceCalibrateableTimeDomainsEXT(
            app()->device().physical_device(), &time_domain_count, nullptr);
    app()->instance()->GetLogger()->LogInfo(
        "Number of calibrateable time domains ", time_domain_count);
    time_domains_.resize(time_domain_count);
    result = app()->instance()->vkGetPhysicalDeviceCalibrateableTimeDomainsEXT(
        app()->device().physical_device(), &time_domain_count,
        time_domains_.data());

    app()->instance()->GetLogger()->LogInfo(
        "Available calibrateable time domains:");
    for (uint32_t i = 0; i < time_domain_count; i++) {
      app()->instance()->GetLogger()->LogInfo(
          "\t", TimeDomainToString(time_domains_[i]));
    }
  ...
  virtual void InitializeFrameData(
      CalibratedTimestampsFrameData* frame_data,
      vulkan::VkCommandBuffer* initialization_buffer,
  ...
  virtual void Render(vulkan::VkQueue* queue, size_t frame_index,
                      CalibratedTimestampsFrameData* frame_data) override {
    ...
    if (frame_count_ % 60 == 0) {
      containers::vector<VkCalibratedTimestampInfoEXT> timestamp_infos(
          time_domains_.size(), {}, data_->allocator());
      for (uint32_t i = 0; i < time_domains_.size(); i++) {
        timestamp_infos[i].sType =
            VK_STRUCTURE_TYPE_CALIBRATED_TIMESTAMP_INFO_EXT;
        timestamp_infos[i].pNext = nullptr;
        timestamp_infos[i].timeDomain = time_domains_[i];
      }
      containers::vector<uint64_t> timestamps(time_domains_.size(), 0,
                                              data_->allocator());
      containers::vector<uint64_t> max_deviation(time_domains_.size(), 0,
                                                 data_->allocator());
      ...
      for (uint32_t i = 0; i < timestamp_infos.size(); i++) {
        app()->GetLogger()->LogInfo(
            "Time domain ", TimeDomainToString(time_domains_[i]),
            ": timestamp ", timestamps[i], ", maxDeviation ", max_deviation[i]);
      }
      app()->GetLogger()->LogInfo("");
      app()->GetLogger()->Flush();
    }
...
int main_entry(const entry::EntryData* data) {
  ...
  CalibratedTimestampsSample sample(data);
  sample.Initialize();
