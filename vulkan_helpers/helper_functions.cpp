...
VkSwapchainKHR CreateDefaultSwapchain(
  ...
  if (device->is_valid()) {
    ...
    uint32_t minSwapchains = std::max(surface_caps.minImageCount + 1, min_image_count + 1);

    LOG_ASSERT(>, instance->GetLogger(), maxSwapchains, minSwapchains);
